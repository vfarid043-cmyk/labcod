import { AdComboConfig, ADCOMBO_LANGUAGES, AdComboTheme } from "@/types/adcombo";
import { useEffect, useState } from "react";

interface AdComboFormPreviewProps {
  config: AdComboConfig;
  language: string;
}

// Theme-specific styles
const getThemeStyles = (theme: AdComboTheme, config: AdComboConfig) => {
  switch (theme) {
    case "neon":
      return {
        card: {
...
export function AdComboFormPreview({ config, language }: AdComboFormPreviewProps) {
  ...
  const lang = ADCOMBO_LANGUAGES[language] || ADCOMBO_LANGUAGES.RO;
    ...
    >
      ...
      >
        <div className="p-5">
          ...
          >
            <div className="font-medium mb-1" style={{ color: config.theme === "premium" ? "#f4e4bc" : config.theme === "neon" ? "#fff" : "#6c757d" }}>Campos Ocultos (AdCombo):</div>
            <div>country_code: {config.countryCode}</div>
            <div>offer_id: {config.offerId || 'OFFER_ID'}</div>
            <div>base_url: {config.baseUrl}</div>
            <div>price: {config.newPrice}</div>
            <div className="truncate">success_page: {config.successPage}</div>
          </div>
