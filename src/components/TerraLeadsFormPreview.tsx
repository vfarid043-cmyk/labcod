import { TerraLeadsConfig, TERRALEADS_LANGUAGES, TerraLeadsTheme } from "@/types/terraleads";
import { useEffect, useState } from "react";

interface TerraLeadsFormPreviewProps {
  config: TerraLeadsConfig;
  language: string;
}

// Theme-specific styles
const getThemeStyles = (theme: TerraLeadsTheme, config: TerraLeadsConfig) => {
  switch (theme) {
    case "neon":
      return {
        card: {
...
export function TerraLeadsFormPreview({ config, language }: TerraLeadsFormPreviewProps) {
  ...
  const lang = TERRALEADS_LANGUAGES[language] || TERRALEADS_LANGUAGES.RO;
  ...
  const getButtonBorderRadius = () => {
    ...
    return radiusMap[config.buttonBorderRadius] || "12px";
  };
  ...
  const getButtonHeight = () => {
    ...
    return sizeMap[config.buttonSize] || "50px";
  };

  return (
    <div
      className="h-full overflow-auto custom-scrollbar flex items-start justify-center p-4 transition-colors"
        ...
        <div className="p-5">
          ...
          {config.showCountdown && (
            <div className="flex justify-center items-center gap-2 mb-5">
              {[
                ...
                <div
                  key={label}
                  className="px-3 py-2 rounded font-bold text-base min-w-[35px] text-center transition-all"
                  style={{
          ...
          <div className={`mb-5 px-2 ${config.showProductImage && config.productImage && config.productImagePosition === 'side' ? 'flex items-center gap-4' : ''}`}>
            ...
            {config.showProductImage && config.productImage && config.productImagePosition === 'side' && (
              <img
                src={config.productImage}
                alt="Product"
                className="w-[100px] h-[100px] object-contain rounded-md shadow-sm flex-shrink-0"
                onError={(e) => {
                  (e.target as HTMLImageElement).style.display = 'none';
                }}
              />
            ...
            <div className="flex justify-between items-center flex-1">
              <div>
                ...
                <div className="line-through text-base" style={themeStyles.priceOld}>
                  {config.oldPrice} {config.currency}
                </div>
              </div>
              <div className="text-right">
                <div className="text-xs mb-1" style={themeStyles.priceOld}>{lang.newPriceLabel}</div>
                <div className="font-bold text-2xl" style={themeStyles.priceNew}>
            ...
            >
              <div className="flex items-center mb-1">
                ...
                <span style={{ color: config.theme === "neon" ? "#ccc" : "#6c757d" }}>
                  {lang.socialProofText}{" "}
                  <span className="font-semibold" style={{ color: config.theme === "neon" ? "#fff" : "#212529" }}>{config.viewingCount}</span>{" "}
                  {lang.socialProofOnline}
                </span>
                <span 
              ...
              <div className="flex items-center">
                ...
                />
                <span style={{ color: config.theme === "neon" ? "#ccc" : "#6c757d" }}>
                  <span className="font-semibold" style={{ color: config.theme === "neon" ? "#fff" : "#212529" }}>{config.salesLastHour}</span>{" "}
                  {lang.salesText}
                </span>
                <span 
                  className="ml-auto animate-pulse"
          ...
          <div className="space-y-4">
            <input
              ...
              className="w-full h-[50px] px-4 rounded-md text-base outline-none focus:ring-2 focus:ring-blue-200 transition-all"
          ...
          {/* Submit Button */}
          <button
            className="w-full text-white font-bold uppercase tracking-wide mt-5 mb-4 transition-all hover:opacity-90"
            style={{
              ...themeStyles.button,
          ...
          <div className="text-center mb-1">
            <span className="mr-1">{config.securityEmoji}</span>
            <span className="text-xs" style={themeStyles.footerText}>{config.securityText}</span>
          </div>
          <div className="text-center text-[10px] leading-tight" style={{ color: config.theme === "neon" ? "#666" : "#adb5bd" }}>
