...
import { generateWebvorkFormHTML, generateWebvorkOrderPHP, generateWebvorkSuccessHTML } from "@/lib/webvork-generators";
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Copy, Check, FileCode, FileText, Server } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

interface WebvorkCodeOutputProps {
  config: WebvorkConfig;
  language: string;
}

export function WebvorkCodeOutput({ config, language }: WebvorkCodeOutputProps) {
  const [copiedTab, setCopiedTab] = useState<string | null>(null);
  const { toast } = useToast();

  const formHTML = generateWebvorkFormHTML(config, language);
  const orderPHP = generateWebvorkOrderPHP(config);
  const successHTML = generateWebvorkSuccessHTML(config);
      ...
      <Tabs defaultValue="form" className="h-full flex flex-col">
        <div className="border-b border-border px-4 pt-4">
          <TabsList className="grid grid-cols-3 bg-secondary/50">
            <TabsTrigger value="form" className="data-[state=active]:bg-orange-600 data-[state=active]:text-white">
              <FileCode className="w-4 h-4 mr-2" />
              index.html
            </TabsTrigger>
            <TabsTrigger value="php" className="data-[state=active]:bg-orange-600 data-[state=active]:text-white">
              <Server className="w-4 h-4 mr-2" />
              order.php
            </TabsTrigger>
            <TabsTrigger value="success" className="data-[state=active]:bg-orange-600 data-[state=active]:text-white">
        ...
        <TabsContent value="form" className="flex-1 flex flex-col m-0 overflow-hidden data-[state=inactive]:hidden">
          <div className="p-3 border-b border-border flex items-center justify-between bg-secondary/30">
            <div className="flex items-center gap-2">
              ...
              <span className="px-1.5 py-0.5 text-xs bg-orange-600/20 text-orange-400 rounded">Webvork</span>
            </div>
        ...
        <TabsContent value="php" className="flex-1 flex flex-col m-0 overflow-hidden data-[state=inactive]:hidden">
          <div className="p-3 border-b border-border flex items-center justify-between bg-secondary/30">
            <div className="flex items-center gap-2">
              ...
              <span className="px-1.5 py-0.5 text-xs bg-blue-600/20 text-blue-400 rounded">API Webvork</span>
            </div>
            <div className="flex gap-2">
              <Button
                size="sm"
