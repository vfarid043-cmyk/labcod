import { WebvorkConfig, WEBVORK_LANGUAGES, WebvorkTheme } from "@/types/webvork";
import { useEffect, useState } from "react";

interface WebvorkFormPreviewProps {
  config: WebvorkConfig;
  language: string;
}

// Theme-specific styles
const getThemeStyles = (theme: WebvorkTheme, config: WebvorkConfig) => {
  switch (theme) {
    case "neon":
      return {
        card: {
...
export function WebvorkFormPreview({ config, language }: WebvorkFormPreviewProps) {
  ...
  return (
    <div
      className="min-h-full flex items-start justify-center p-4 overflow-auto custom-scrollbar transition-colors"
    ...
    >
      ...
      >
        <div className="p-5">
          ...
          {/* Product Image Top */}
          {config.showProductImage && config.productImagePosition === "top" && config.productImage && (
          ...
          {/* Price Container */}
          <div className={`my-5 ${config.showProductImage && config.productImagePosition === "side" && config.productImage ? "flex items-center gap-4" : ""}`}>
            {config.showProductImage && config.productImagePosition === "side" && config.productImage && (
              <img
                src={config.productImage}
                alt="Product"
            ...
            <div className="flex justify-between items-center flex-1">
              <div>
                ...
                <div className="line-through" style={themeStyles.priceOld}>
          ...
          {config.showSocialProof && (
            <div 
              className="my-5 text-xs space-y-1"
            ...
            >
              <div className="flex items-center">
                ...
                <span style={{ color: config.theme === "neon" ? "#ccc" : "#6c757d" }}>
                  ...
                  {language === "RO" ? "persoane" : language === "PL" ? "osób" : language === "RU" ? "человек" : "pessoas"}
                </span>
                <span 
          ...
          <div className="space-y-4">
            ...
            <button
              className="w-full h-12 mt-5 mb-4 border-none rounded-full text-white text-base font-bold uppercase tracking-wide cursor-pointer transition-all hover:brightness-90"
            ...
            <div className="text-center text-[10px] mt-2" style={{ color: config.theme === "neon" ? "#666" : "#adb5bd" }}>
